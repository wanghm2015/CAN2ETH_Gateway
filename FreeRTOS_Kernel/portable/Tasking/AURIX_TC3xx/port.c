...
#include "FreeRTOS.h"
#include "task.h"
#include "Libraries/iLLD/TC3xx/Tricore/Cpu/Std/Platform_Types.h"
...
#ifdef TEST_CODE
TCB_t * volatile test_watch_TCB = NULL;
uint32 prev_stm_value = 0;
uint32 current_stm_value = 0;
uint32 stm_diff = 0;
float32 actual_delay_ms = 0.0f;
uint32 cmp0_value = 0;
uint32 next_cmp0_value = 0;
#endif
...
vPortSystemContextHandler()
{
    ...
    #ifdef TEST_CODE
    test_watch_TCB = ( TCB_t * )pxCurrentTCB;
    #endif
...
vPortSystemTickHandler()
{
    ...
    #else
    pxStm[ portSTM_CMP0 >> 2 ] = pxStm[ portSTM_TIM0 >> 2 ] + portTICK_COUNT;    //whm
    #endif
...
void __attribute__( ( noinline, used ) ) vPortSyscallYield()
{
    /* Do a save, switch, execute */
...
#endif
vPortSyscallHandler( IfxCpu_Trap trapWatch )
{
...
{
    #ifdef TEST_CODE
    prev_stm_value = pxStm[ portSTM_TIM0 >> 2 ];
    #endif
