...
/*-----------------------------------------------------Macros--------------------------------------------------------*/
/*********************************************************************************************************************/
#define LED_1                   IfxPort_P33_5               /* Port/Pin for LED 1                                    */
#define LED1_BLINKY_PERIOD_MS   (1000)                       /* The period (in milliseconds) at which LED1 will blink */

uint64_t whm_test;
#ifdef TEST_CODE
extern List_t xDelayedTaskList1; 
ListItem_t * test_next_item; 
uint32_t xTickCount_whm_test_pre = 0;
uint32_t xTickCount_whm_test_diff = 0;
#endif
/*********************************************************************************************************************/
...
void task_blinky(void *arg)
{
    ...
    while (1)
    {
        ...
        #ifdef TEST_CODE
        xTickCount_whm_test_diff = xTaskGetTickCount() - xTickCount_whm_test_pre;
        xTickCount_whm_test_pre = xTaskGetTickCount();

        taskENTER_CRITICAL();
        test_next_item = xDelayedTaskList1.pxIndex->pxNext;
        taskEXIT_CRITICAL();
        #endif
